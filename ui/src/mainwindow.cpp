...
void MainWindow::setupUi()
{
    ...
    timeSlider_ = new QSlider(Qt::Horizontal, topControlsAndTimeline);
    timeSlider_->setEnabled(false);
    topLayout->addWidget(timeSlider_);
    ...
    QAction *openSnapshotAction = fileMenu->addAction(tr("Open Snapshot…"));
    ...
    connect(openSnapshotAction, &QAction::triggered, this, &MainWindow::openSnapshot);
    ...
    connect(timeSlider_, &QSlider::valueChanged, this, &MainWindow::handleTimeScrub);
}
...
{
    if (dataMode_ == DataMode::Snapshot) {
        return;
    }
...
void MainWindow::openSnapshot()
{
    const QString path = QFileDialog::getOpenFileName(
        this,
        tr("Open KPulse Snapshot"),
        QString(),
        tr("KPulse Snapshots (*.json)"));
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        QMessageBox::critical(this, tr("Error"), tr("Failed to open snapshot file."));
        return;
    }

    QJsonDocument doc = QJsonDocument::fromJson(f.readAll());
    if (!doc.isObject()) {
        QMessageBox::critical(this, tr("Error"), tr("Invalid snapshot format."));
        return;
    }

    QJsonObject root = doc.object();
    if (!root.contains(QStringLiteral("events")) || !root.value(QStringLiteral("events")).isArray()) {
        QMessageBox::critical(this, tr("Error"), tr("Snapshot does not contain events."));
        return;
    }

    std::vector<kpulse::Event> events;
    for (const auto &value : root.value(QStringLiteral("events")).toArray()) {
        if (value.isObject()) {
            events.push_back(kpulse::eventFromJson(value.toObject()));
        }
    }

    snapshotEvents_ = events;
    dataMode_ = DataMode::Snapshot;
    disconnect(&ipc_, nullptr, this, nullptr);

    refreshButton_->setEnabled(false);
    rangeCombo_->setEnabled(false);

    timeSlider_->setEnabled(!snapshotEvents_.empty());
    timeSlider_->setRange(0, 1000);
    timeSlider_->setValue(1000);

    model_->setEvents(snapshotEvents_);
    timelineView_->setEvents(snapshotEvents_);

    detailsEdit_->clear();
    statusBar()->showMessage(tr("Snapshot loaded (%1 events)").arg(snapshotEvents_.size()));
}

void MainWindow::handleTimeScrub(int value)
{
    if (dataMode_ != DataMode::Snapshot) {
        return;
    }

    if (snapshotEvents_.empty()) {
        return;
    }

    const QDateTime minT = snapshotEvents_.front().timestamp;
    const QDateTime maxT = snapshotEvents_.back().timestamp;
    const qint64 span = minT.msecsTo(maxT);
    const qint64 cutoff = minT.toMSecsSinceEpoch() + (span * value) / 1000;

    const QString needle = searchEdit_->text().trimmed().toLower();

    auto categoryAllowed = [&](kpulse::Category category) {
        switch (category) {
        case kpulse::Category::System:
            return checkboxSystem_->isChecked();
        case kpulse::Category::GPU:
    ...
    std::vector<kpulse::Event> visible;
    for (const auto &ev : snapshotEvents_) {
        if (ev.timestamp.toMSecsSinceEpoch() > cutoff) {
            continue;
        }
        if (!categoryAllowed(ev.category)) {
            continue;
        }
        if (!needle.isEmpty()) {
            const QString label = ev.label.toLower();
            const QString message = ev.details.value(QStringLiteral("message")).toString().toLower();
            if (!label.contains(needle) && !message.contains(needle)) {
                continue;
            }
        }
    ...
    model_->setEvents(visible);
    timelineView_->setEvents(visible);
}
